...
@Tag(name = "Appointment", description = "의사 예약 관련 API")
public interface DoctorAppointmentApiDocs {

    @Operation(
            summary = "의사 예약 목록 조회",
            description = "의사 본인의 예약 목록을 커서 페이징으로 조회합니다."
    )
    @ApiResponses({
    ...
    CursorPageResponse<DoctorAppointmentListResponse> getAppointmentList(@Parameter(hidden = true) CurrentUser currentUser,
                                                                         @Valid DoctorAppointmentListRequest request);

    @Operation(
            summary = "의사 예약 조회",
            description = "의사 본인의 예약을 선택하여 id를 통해 조회합니다."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공"),
            @ApiResponse(responseCode = "400", description = "잘못된 요청 파라미터"),
    ...
    DoctorAppointmentDetailResponse getAppointmentDetail(@PathVariable Long appointmentId,
